...
public class SpringTest extends BaseTest {
    ...
    public void testPmt() {
        ...
        context.set("MR", CalculatorUtil.parseRule("YR.div(12,PR)", BigDecimal.class, context)); // 月利率


        String ma = "LA.mul(MR).mul(MR.add(1).pow(LM)).div(MR.add(1).pow(LM).sub(1),PR).off(RS)";

        System.out.println(CalculatorUtil.parseRule(ma, BigDecimal.class, context));
    }
}
